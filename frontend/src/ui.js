...
const Flow = ({ onRegisterTapAdd }) => {
  ...
  // Offset tracker so multiple tapped nodes don't stack exactly
  const tapOffsetRef = useRef(0);

  // Called when user TAPS a node button in toolbar (mobile)
  const handleTapAdd = useCallback((type) => {
    tapOffsetRef.current = (tapOffsetRef.current + 40) % 200;
    const offset = tapOffsetRef.current;
  ...
  // Expose handleTapAdd up to App via callback ref pattern
  if (onRegisterTapAdd) onRegisterTapAdd(handleTapAdd);
  ...
  return (
    <div ref={wrapperRef} className="pipeline-canvas">
      <ReactFlow
        ...
        maxZoom={2}
      >
        <Background color="#cbd5e1" gap={GRID} variant="dots" />
